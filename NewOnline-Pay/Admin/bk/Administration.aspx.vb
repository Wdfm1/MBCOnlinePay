...
Partial Class Admin_Administration
    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        Else
            dsorder.SelectCommand = "SELECT orders.teacher, orders.itemamount, orders.itemqty, orders.orderid, orders.orddate, orders.grade, orders.studentfname, orders.studentlname, orders.itemtotal, orders.booktype, orders.emailaddress,   orders.perstext1, orders.perstext2, orders.paytype, orders.schname, payment.transid FROM orders LEFT OUTER JOIN payment ON orders.orderid = payment.orderid WHERE (orders.schcode = @schcode) and (orders.yr = '14') and ORDERS.orderid=@orderid AND (payment.orderid IS NOT NULL) ORDER BY orders.orddate, orders.orderid;"
            ViewState("custcmd") = dsorder.SelectCommand
    ...
    Protected Sub Button2_Click(sender As Object, e As System.EventArgs) Handles Button2.Click
        ...
        If txtstudentname.Text = Nothing Then
            ...
            ViewState("custcmd") = dsorder.SelectCommand
        Else
            dsorder.SelectCommand = "SELECT orders.teacher, orders.itemamount, orders.itemqty, orders.orderid, orders.orddate, orders.grade, orders.studentfname, orders.studentlname, orders.itemtotal, orders.booktype, orders.emailaddress,  orders.perstext1, orders.perstext2, orders.paytype, orders.schname, payment.transid  FROM orders LEFT OUTER JOIN payment ON orders.orderid = payment.orderid WHERE (orders.schcode = @schcode) and upper(orders.studentlname)=upper(@studentlname)and(orders.yr = '14') AND (payment.orderid IS NOT NULL) ORDER BY orders.orddate, ORDERS.orderid ;"
            ViewState("custcmd") = dsorder.SelectCommand
